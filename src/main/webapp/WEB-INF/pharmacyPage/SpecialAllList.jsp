...
<body>
<div class="x-nav"> <span class="layui-breadcrumb"> <a><cite>首页</cite></a> <a><cite>麻醉精神药品明细</cite></a> </span> <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right"  href="javascript:location.replace(location.href);" title="刷新"><i class="layui-icon" style="line-height:30px">ဂ</i></a> </div>
<div class="x-body">
  ...
  <table class="tablelist">
      ...
      <c:when test="${not empty requestScope.specialAllPageList.list}">
        <c:forEach items="${requestScope.specialAllPageList.list}" var="specialAllPageList">
            <tr>
              ...
              <td>${specialAllPageList.drugQuantity}</td>
              <td>${specialAllPageList.approvalnumber}</td>
              <td>${specialAllPageList.produtionDate}</td>
              <td>${specialAllPageList.validaityperiod}</td>
              <td>${specialAllPageList.storageTime}</td>
            </tr>
        </c:forEach>
      </c:when>
      <c:otherwise>
    ...
    <tr>
      ...
      <td colspan="9" align="right">
        <a href="/anesthesia/anesthesiaAllList.action?pageNum=1&&state=1"    target="main">首页</a>
        <a href="/anesthesia/anesthesiaAllList.action?pageNum=${specialAllPageList.prePage}&&state=1"  target="main">上一页</a>
        <a href="/anesthesia/anesthesiaAllList.action?pageNum=${specialAllPageList.nextPage}&&state=1"  target="main">下一页</a>
        <a href="/anesthesia/anesthesiaAllList.action?pageNum=${specialAllPageList.navigateLastPage}&&state=1"  target="main">尾页</a>
      </td>
    </tr> 
    </tbody>
  </table>
