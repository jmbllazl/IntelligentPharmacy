...
<body>
<div class="x-nav"> <span class="layui-breadcrumb"> <a><cite>首页</cite></a> <a><cite>药房盘点统计</cite></a> </span> <a class="layui-btn layui-btn-small" style="line-height:1.6em;margin-top:3px;float:right"  href="javascript:location.replace(location.href);" title="刷新"><i class="layui-icon" style="line-height:30px">ဂ</i></a> </div>
<div class="x-body">
      ...
      <c:when test="${not empty requestScope.drugpage.list}">
        ...
        <c:forEach items="${requestScope.drugpage.list}" var="drugTemp">
            <tr>
              ...
              <td>
                <input type="button" value="查看详情" onclick="user_management_show('药品信息','/phamacy/phamacyDrugDetils.action?pharmacyId=${drugTemp.pharmacyId}','600','500')">
            </tr>
        </c:forEach>  </form>
      </c:when>
      <c:otherwise>
  ...
  <tr>
      ...
      <td colspan="8" align="right">
        <a href="${pageContext.request.contextPath}/report/reportAllDrugs.action?num=1"    target="main">首页</a>
        <a href="${pageContext.request.contextPath}/report/reportAllDrugs.action?num=${drugpage.prePage}"   target="main">上一页</a>
        <a href="${pageContext.request.contextPath}/report/reportAllDrugs.action?num=${drugpage.nextPage}"   target="main">下一页</a>
        <a href="${pageContext.request.contextPath}/report/reportAllDrugs.action?num=${drugpage.navigateLastPage}"    target="main">尾页</a>
      </td>
    </tr> 
    </tbody>
  </table>
...
<script>
    function chageNum(num) {
        console.log(num);
        var form = $("form");
        form.action="${pageContext.request.contextPath}/report/reportAllDrugs.action";
